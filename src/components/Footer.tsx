...
const productsList = ["Tiger Prawns","Vannamei Shrimp","Cuttlefish","Squid","Octopus","Reef Cod (Grouper)"];
const certsList     = ["HACCP","USFDA","BRCGS","FSSAI","EU Approved","BAP"];
const markets       = [{ region: "Europe", flag: "🇪🇺" },{ region: "United States", flag: "🇺🇸" },{ region: "Far East", flag: "🌏" },{ region: "Australia", flag: "🇦🇺" }];

const scrollTo = (href: string) => document.querySelector(href)?.scrollIntoView({ behavior: "smooth", block: "start" });

const SocialLink = ({ href, label, bg, icon }: { href: string; label: string; bg: string; icon: React.ReactNode }) => (
  <a href={href} target="_blank" rel="noopener noreferrer" aria-label={label}
    style={{ width: "36px", height: "36px", borderRadius: "8px", background: bg, display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none", transition: "all 0.3s ease", flexShrink: 0 }}
  ...
  return (
    <footer style={{ background: "var(--ocean-900)", borderTop: "1px solid rgba(34,211,238,0.08)", position: "relative", overflow: "hidden" }}>
      ...
      <div className="container-xl" style={{ paddingTop: "5rem", paddingBottom: "3rem", position: "relative", zIndex: 2 }}>
        ...
        {/* ─ Main grid: rg-footer: 4col → 2col tablet → 1col mobile ─ */}
        <div className="rg-footer" style={{ marginBottom: "4rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <p style={{ fontSize: "0.875rem", color: "var(--text-secondary)", lineHeight: 1.8, maxWidth: "320px" }}>
              Kerala&apos;s premier frozen seafood exporter since 2004. Delivering premium IQF and Block
              Frozen seafood to global markets with uncompromising quality and sustainability.
            </p>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              <a href="tel:+919846079833" style={{ fontSize: "0.85rem", color: "var(--text-secondary)", textDecoration: "none", transition: "color 0.3s" }} onMouseEnter={e => (e.currentTarget.style.color = "var(--cyan-400)")} onMouseLeave={e => (e.currentTarget.style.color = "var(--text-secondary)")}>📞 +91 98460 79833</a>
              <a href="mailto:abmmarineproducts@gmail.com" style={{ fontSize: "0.85rem", color: "var(--text-secondary)", textDecoration: "none", transition: "color 0.3s", wordBreak: "break-word" }} onMouseEnter={e => (e.currentTarget.style.color = "var(--cyan-400)")} onMouseLeave={e => (e.currentTarget.style.color = "var(--text-secondary)")}>✉️ abmmarineproducts@gmail.com</a>
              <span style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>📍 Palluruthy, Kochi - 682006, Kerala</span>
            </div>

            {/* Social */}
            <div style={{ display: "flex", gap: "0.75rem" }}>
              <SocialLink href="https://wa.me/919846079833" label="WhatsApp" bg="rgba(37,211,102,0.15)"
              ...
              />
              <SocialLink href="https://linkedin.com" label="LinkedIn" bg="rgba(10,102,194,0.15)"
                icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="#0a66c2"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z"/><circle cx="4" cy="4" r="2"/></svg>}
              />
              <SocialLink href="mailto:abmmarineproducts@gmail.com" label="Email" bg="rgba(34,211,238,0.1)"
                icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--cyan-400)" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.65rem", marginBottom: "2rem" }}>
              {productsList.map(p => (
                <li key={p}>
                  <a href="#products" onClick={(e) => { e.preventDefault(); scrollTo("#products"); }} style={{ fontSize: "0.875rem", color: "var(--text-secondary)", textDecoration: "none", transition: "color 0.3s" }} onMouseEnter={e => (e.currentTarget.style.color = "var(--teal-400)")} onMouseLeave={e => (e.currentTarget.style.color = "var(--text-secondary)")}>{p}</a>
                </li>
              ))}
            </ul>
            ...
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.6rem" }}>
              {markets.map(m => (<li key={m.region} style={{ fontSize: "0.875rem", color: "var(--text-secondary)" }}>{m.flag} {m.region}</li>))}
            </ul>
          </div>
