...
const timelineItems = [
  { year: "2004", title: "Founded in Kochi",    desc: "Established by Mr. P.A. Xavier and three industry veterans near Kochi's fishing harbors, Kerala." },
  { year: "2008", title: "First Global Export", desc: "Expanded to international markets, building early partnerships in Europe and the Far East." },
  { year: "2015", title: "HACCP & USFDA",       desc: "Achieved critical international certifications, unlocking North American and global premium markets." },
  { year: "2024", title: "Global Network",       desc: "Supplying premium frozen seafood to Europe, USA, Far East, and Australia with trusted consistency." },
];
...
export default function AboutSection() {
  return (
    <section
      ...
      style={{ background: "var(--ocean-800)", position: "relative", overflow: "hidden" }}
    >
      ...
      <div className="container-xl" style={{ position: "relative", zIndex: 2 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "4rem" }}>
          <div className="label-tag" style={{ display: "inline-flex", marginBottom: "1.25rem" }}>Our Story</div>
          <h2 className="section-heading" style={{ maxWidth: "700px", margin: "0 auto 1.25rem" }}>
            Two Decades of{" "}<span className="gradient-text">Kerala Seafood</span>{" "}Excellence
          </h2>
        ...
        {/* Split layout — rg-split handles 2col → 1col */}
        <div className="rg-split" style={{ marginBottom: "5rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            ...
            <p style={{ color: "var(--text-secondary)", lineHeight: 1.9 }}>
              ...
              satisfaction. Our dedication transformed us from a small exporter into a trusted international brand.
            </p>
            <p style={{ color: "var(--text-secondary)", lineHeight: 1.9 }}>
              Today, we supply premium seafood to markets across{" "}
            ...
            {/* Highlights — rg-2-tight → 1col on mobile */}
            ...
            {/* Vision & Mission — rg-vm → 1col on mobile */}
            <div className="rg-vm">
        ...
        {/* Timeline — rg-timeline: 4col → 2col tablet → 1col mobile */}
        <div style={{ marginBottom: "4rem" }}>
          <div style={{ textAlign: "center", marginBottom: "3rem" }}>
            <h3 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.4rem, 3vw, 2rem)", fontWeight: 700, color: "var(--text-primary)" }}>Our Journey</h3>
          </div>
          <div className="rg-timeline">
            {timelineItems.map((item) => (
              <div key={item.year} className="glass-card" style={{ padding: "1.75rem 1.25rem", borderRadius: "16px", textAlign: "center" }}>
                ...
                <div style={{ fontFamily: "var(--font-display)", fontSize: "1.5rem", fontWeight: 900, color: "var(--cyan-400)", marginBottom: "0.5rem" }}>{item.year}</div>
                <div style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: "0.9rem", color: "var(--text-primary)", marginBottom: "0.5rem" }}>{item.title}</div>
                <p style={{ fontSize: "0.78rem", color: "var(--text-secondary)", lineHeight: 1.6 }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: "0" }}>
            {["Quality", "Trust", "Sustainability", "Innovation"].map((val, i, arr) => (
              <span key={val} style={{ display: "flex", alignItems: "center" }}>
                ...
                {i < arr.length - 1 && <span style={{ color: "var(--cyan-400)", opacity: 0.5, fontSize: "1.2rem" }}>·</span>}
              </span>
            ))}
          </div>
        </div>
