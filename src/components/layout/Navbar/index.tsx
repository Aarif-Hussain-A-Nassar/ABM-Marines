...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    // Only observe links that exist as sections on the home page
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // Match exactly with hash or match page URL with section ID
            const matchingLink = navLinks.find(l => 
              l.href.endsWith(`#${activeId}`) || 
              l.href === `/${activeId}` ||
              (l.href === "/sustainability" && activeId === "sustainability") ||
              (l.href === "/contact" && activeId === "contact")
            );
            
            if (matchingLink) {
              setActive(matchingLink.href);
            }
          }
        });
      },
      { 
        ...
        rootMargin: "-10% 0px -70% 0px" // Focus on the top-third of the viewport
      }
    );
